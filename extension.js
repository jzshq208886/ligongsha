...
content:function(config,pack){
	...
	get.rawDistance=function(player,target,direction){
		var X1=0,X2=0
		var current=player;
		...
		if(direction=='right') return X1;
		current=player;
		while(current!=target){
			X2+=1;
			current=current.previous;
		}
		if(direction=='left') return X2;
...
return {
	...
	character:{
		...
		"lgs_yuxiaoran":["female","shen","2/3",["lgs_jingshu","lgs_duyi"],['ext:理工杀/lgs_yuxiaoran.jpg','db:extension-理工杀:lgs_yuxiaoran.jpg']],
		"lgs_wanglinjing":["female","shen",3,["lgs_xiaokan","lgs_caisi"],['ext:理工杀/lgs_wanglinjing.jpg','db:extension-理工杀:lgs_wanglinjing.jpg']],
		...
		"sp_wangyixuan":["male","qun",4,["lgs_touliang","lgs_huanzhu"],['ext:理工杀/sp_wangyixuan.jpg','db:extension-理工杀:sp_wangyixuan.jpg']],
	},
	characterTitle:{
		"sp_hankuncheng":"怀勇兼柔",
		"sp_zhuxinkun":"健身达人",
	...
	skill:{  
			...
			subSkill:{
				buff:{
					...
					content:function(){
						...
						'step 1'
						var time=400-(get.utc()-event.time);
						if(time>0){
							game.delay(0,time);
						}
						'step 2'
		...
		"lgs_xiyan":{
			init:function(player){
				player.storage.lgs_xiyan_disable=[];
			},
			enable:'chooseToUse',
			hiddenCard:function(player,name){
			...
			chooseButton:{
				...
				filter:function(button){
					if(_status.event.player.storage.lgs_xiyan_disable.contains(button.link[2])) return false;
					return true;
				},
				check:function(button){
		...
		lgs_jingshu:{
			...
			content:function(){
				...
				player.chooseCardButton([1,Infinity],trigger.cards2,'静姝：获得任意张没有“静姝”标记的牌').set('filterButton',function(button){
					...
					return get.value(button.link)-0.5;
				});
				'step 2'
				var time=300-(get.utc()-event.time);
				if(time>0){
					game.delay(0,time);
				}
				'step 3'
			...
			subSkill:{
				discard:{
					...
					direct:true,
					content:function(){
						var origin=trigger.ai
						trigger.ai=function(card){
							return origin(card)-(card.hasGaintag('lgs_jingshu')?2:0);
						};
					},
				},
			},
		...
		lgs_duyi:{
			...
			enable:'phaseUse',
			filter:function(event,player){
				return !player.hasSkill('lgs_duyi_block')&&(player.countCards('h')||player.getExpansions('lgs_duyi').length);
			},
			log:false,
			delay:false,
			step0:function(event,player){
				var yi=player.getExpansions('lgs_duyi').length
				if(!yi) event.goto(2);
				else if(!player.countCards('h')) event.goto(5);
				else {
					event.two=true;
					player.chooseControl('放置“毅”','获得“毅”','cancel').set('ai',function(evt,player){  // 如果选择cancel，result.control为undefined且result.bool为false
						// 如果毅大于hp，获得毅；
						// 如果毅小于hp，
						//   如果毅与hp差为1或有适合放置的牌，放置毅；
						//   否则取消；
						// 如果毅等于hp，
						//   如果有适合放置的牌，放置毅；
						//   否则获得毅
						var yi=_status.event.yi
						if(yi>player.hp){
							return 1;
						}else if(yi<player.hp){
							if(player.hp-yi==1||player.hasCard(function(card){
								return 6-get.value(card)+(card.hasGaintag('lgs_jingshu')?3:0)>0;
							},'h')) return 0;
							return 2;
						}else{
							if(player.hasCard(function(card){
								return 6-get.value(card)+(card.hasGaintag('lgs_jingshu')?2:0)>0;
							},'h')) return 0;
							return 1;
						}
					}).set('yi',yi).set('prompt',get.prompt2('lgs_duyi'));
				}
			},
			step1:function(event,player,result){
				if(result.bool===false) event.finish();
				else if(result.control=='获得“毅”') event.goto(5);
			},
			step2:function(event,player){
				player.chooseCard('h').set('ai',function(card){
					if(player.hp-player.getExpansions('lgs_duyi').length==1)
						return 999-get.value(card)+(card.hasGaintag('lgs_jingshu')?2:0);
					return 6-get.value(card)+(card.hasGaintag('lgs_jingshu')?2:0);
				}).set('prompt','笃毅：将一张手牌置于武将牌上当作“毅”');
			},
			step3:function(event,player,result){
				if(result.bool){
			...
			step4:function(event,player){
				player.draw();
				event.goto(7);
			},
			step5:function(event,player){
				player.chooseCardButton('笃毅：获得任意张“毅”',[1,Infinity],player.getExpansions('lgs_duyi')).set('ai',function(button){
					var yi=player.getExpansions('lgs_duyi').length
					if(yi<player.hp) return 0;
					if(yi==player.hp){
						if(ui.selected.buttons.length) return 0;
						return get.value(button.link);
					}
					if(ui.selected.buttons.length>=yi-player.hp) return 0;
					return get.value(button.link);
				});
			},
			step6:function(event,player,result){
				if(result.bool){
					player.logSkill('lgs_duyi');
					game.delay(0.2);
			...
			step7:function(event,player){
				if(player.getExpansions('lgs_duyi').length==player.hp)
					player.recover();
			},
			content:function(){
				'step 0'
				lib.skill.lgs_duyi.step0(event,player);
				'step 1'
				lib.skill.lgs_duyi.step1(event,player,result);
				'step 2'
				lib.skill.lgs_duyi.step2(event,player);
				'step 3'
				lib.skill.lgs_duyi.step3(event,player,result);
				'step 4'
				lib.skill.lgs_duyi.step4(event,player);
				'step 5'
				lib.skill.lgs_duyi.step5(event,player);
				'step 6'
				lib.skill.lgs_duyi.step6(event,player,result);
				'step 7'
				lib.skill.lgs_duyi.step7(event,player);
			},
			ai:{
				maixie:true,
				maixie_hp:true,
				...
				result:{
					player:function(player){
						var yi=player.getExpansions('lgs_duyi').length
						if(yi&&yi>=player.hp) return 1;
						if(player.hp-yi==1&&player.countCards('h')||player.hasCard(function(card){
							return 6-get.value(card)+(card.hasGaintag('lgs_jingshu')?2:0)>0;
						},'h')) return 1;
						return 0;
					},
		...
		lgs_xiaokan:{
			...
			content:function(){
				'step 0'
				player.discardPlayerCard(target,'he').set('ai',function(button){
					var val=get.buttonValue(button)+(get.position(button.link)=='h'?2:0);
				...
				next.set('ai',function(){
					...
					if(get.effect(jing,{name:'sha'},player,player)<=0) return 1;
					var num=Math.min(4,player.countCards('h'))
					if(player.isTurnedOver()) return 1;
					if(get.attitude(player,jing)>0) return 1;
					if(player.hp>1&&(!player.countCards('he')||jing.hp==1&&!jing.mayHaveShan())) return 0;
					if(player.hp<=1) return 1;
					if(num>=4) return 1;
					if(num>=3){
						if(player.hp>2) {
							if(jing.hp<2) return [1,0].randomGet();
							if(jing.hp<3) return [1,1,1,0].randomGet();
							return [1,1,1,1,0].randomGet();
						}
						return 1;
					}
					if(num>=2){
						if(player.hp>2) return [1,1,0].randomGet();
						return [1,1,1,0].randomGet();
					}
					if(num>=1){
					...
					if(player.hp<=1) return 1;
					if(!player.countCards('e')||player.hp>2) return 0
					return [1,0,0].randomGet();
				});
				'step 2'
				if(result.index==1)
					event.goto(4);
			...
			ai:{
				...
				result:{
					...
					target:function(player,target){
						if(target.isTurnedOver()) return 1+Math.min(4,target.countCards('h'))
						var effect=-4+0.5*(target.countCards('h')+target.hp);
						// game.log('xiaokan-result-target:', target, ', ', effect);
						return effect;
					},
				},
			},
		...
		lgs_caisi:{
			...
			// 		return '将'+get.translation(ui.selected.cards[0])
			// 			+'当做'+get.translation(lib.skill.lgs_caisi.viewAsMap[ui.selected.cards[0].name])
			// 			+(event.name=='chooseToUse'?'使用':'打出');
			// 	}
			// 	return get.prompt('lgs_caisi');
			// },
			viewAs:function(cards,player){
				return lib.skill.lgs_caisi.viewAsMap[cards[0].name]
				// var name=false;
				// var nature=null;
			...
			check:function(card){
				...
				if(_status.event.type=='phase'){
					var max=0;
					var name2;  // 用来记录实体牌名
					var list=['shan','shandian','sha','huogong','tao','taoyuan','wuxie','wuzhong'];
					for(var i=0;i<list.length;i++){  // 求有实用价值的order最大的牌名
						...
						// 	}
						// }
						var name=list[i];  // 待转换的实体牌名
						var viewAs=lib.skill.lgs_caisi.viewAsMap[name]
						if(player.countCards('hes',function(card){
							if(get.value(card)>=7) return false;
							if(name=='sha'&&get.nature(card,player)!='fire') return false;
							return get.name(card,player)==name;
						})>0&&player.getUseValue(viewAs)>0){
							var temp=get.order(viewAs);
							if(temp>max){
								max=temp;
								name2=name;
							}
			...
			filterCard:function(card,player,event){
				...
				for(var name2 of list){
					...
					if(name=='sha'&&nature!='fire') continue;
					if(filter({name:viewAs.name,nature:viewAs.nature,cards:[card]},player,event))
						return true;
				}
			...
			filter:function(event,player){
				...
				for(var card of list){
					var viewAs=lib.skill.lgs_caisi.viewAsMap[card]
					if(card=='sha') card={name:'sha',nature:'fire'};
					if(filter(viewAs)&&player.countCards('hes',card)) return true;
				}
		...
		lgs_minghao:{
			...
			subSkill:{
				...
				aistorage:{
					...
					popup:false,
					filter:function(event,player){
						game.log('minghao_aistorage-filter');
						// game.log(event.visible);
						game.log('event.animate: ',event.animate);
						return event.player!=player&&['give','gain','gain2'].contains(event.animate);
					},
					forced:true,
					content:function(){
						game.log('minghao_aistorage-in');
						game.log('trigger.player.playerid: ',trigger.player.playerid);
		...
		lgs_fenqu:{
			...
			check:function(event,player){
				var cardx=get.copyBySkill(event.card,'lgs_fenqu')
				return get.effect(event.target,cardx,player,player)<=8||event.target.countCards('he')<4;
			},
			content:function(){
				'step 0'
			...
			ai:{
				effect:{
					player:function(card,player,target){
						if(card.lgs_fenqu_copy) return;
						if(get.name(card)=='sha'&&get.attitude(player,target)<0) return [1,1];
					},
				},
			},
		},
		...
		lgs_touliang1:{
			...
			content:function(){
				...
				player.draw();
				'step 1'
				if(event.triggername.endsWith('Skipped')){
					var name=event.triggername.slice(0,-7);
					game.log(player,'获得了额外的',name);
					var next=player[name]();
					event.next.remove(next);
					trigger.next.push(next);
				}else{
					game.log(player,'获得了额外的',trigger.name);
		...
		lgs_huanzhu:{
			...
			content:function(){
				...
				if(!player.isAlive())
					...
					player.draw(get.rawDistance(player,trigger.player));
				}
				'step 2'
				game.delayx(0.3);
				'step 3'
	...
	translate:{
		...
		"lgs_xiyan":"戏言",
		"lgs_xiyan_info":"当你想要使用【杀】或非延时锦囊牌指定一名其他角色为唯一目标时，你可以令该角色猜测你手牌中是否有此牌：若有且其猜测没有，你获得其一张牌；若没有且其猜测有，你视为对其使用了此牌；若其猜对，本回合你不能再发动〖戏言〗。你发动〖戏言〗不可选择本回合选择过的牌名。",
		"lgs_xingchi":"行迟",
